...
const Navbar = () => {
  ...
  return (
    <nav className={`bg-secondary w-full flex justify-between items-center fixed z-50 gap-5 p-6 sm:px-12 ${theme === themes.myLight ? 'light-shadow' : ''}`}>
        <Link to="/" className='text-[24.85px]'>
          Coll<span className="text-primary ">App</span>
        </Link>
        <input type="search" placeholder="Search anything globally..." className={`input input-bordered w-full max-w-[600px] max-lg:hidden ${theme === themes.myLight ? 'bg-base-300' : 'bg-neutral'} placeholder:text-neutral-content`} />
